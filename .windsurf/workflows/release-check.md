---
description: Pre-release verification checklist for stream-join
---
...
1. Check that `src/index.js` and `src/index.d.ts` are in sync (the exported function and option shapes match).
...
7. Verify `package.json`:
   ...
   - `dependencies` contains only `stream-chain`.
